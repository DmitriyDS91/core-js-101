...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const str1 = isStartIncluded ? '[' : '(';
  const str2 = isEndIncluded ? ']' : ')';
  return b > a ? `${str1}${a.toString()}, ${b.toString()}${str2}` : `${str1}${b.toString()}, ${a.toString()}${str2}`;
}
...
function getDigitalRoot(num) {
  ...
  return sum % 10 === sum ? sum : getDigitalRoot(sum);
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < string.length; i += 1) {
    for (let j = 0; j < bracketsConf.length; j += 1) {
      if (string[i] === bracketsConf[j][0] && string[i + 1] === bracketsConf[j][1]) {
